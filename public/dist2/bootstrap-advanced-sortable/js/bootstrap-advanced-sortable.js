    ...
    BootstrapTable.prototype.initHeader = function () {
        if (this.options.advancedSortable) {
                ...
                if (that.options.sortable) {
                    ...
                    if (sort_model.length === 0) {
                        ...
                        let sortHtml = ` <ul class="nav-sort">
                                            <li><a class="sort-asc" href="#"><i class="${ascIcon}"></i> <font size="2">升序排序</font></a></li>
                                            <li><a class="sort-desc" href="#"><i class="${descIcon}"></i> <font size="2">降序排序</font></a></li>
                                        </ul>`;
                        if (searchT === "num") {
            ...
            this.$container.off('click', '.sort-asc').on('click', '.sort-asc', function (event) {
                ...
                let btn = `<buuton class="btn-sort btn-remove-sort btn-sort-position" value="${field}" style="font-size: 13px"> <font size="2">${title} 升序</font> ${closeIcon}</buuton>`
            ...
            this.$container.off('click', '.sort-desc').on('click', '.sort-desc', function (event) {
                ...
                let btn = `<buuton class="btn-sort btn-remove-sort btn-sort-position" value="${field}" style="font-size: 13px"><font size="2">${title} 降序</font>  ${closeIcon}</buuton>`
                that.$advancedSortable.find(".sort-btn-toolbar").html(btn);
                //   $("#toolbar").find(".sort-btn-toolbar").html(btn);
                $(".sort-toolbar").show();
                $(".sort-model").hide()
